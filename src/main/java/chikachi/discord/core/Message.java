...
public class Message {
    ...
    chikachi.discord.core.WebhookMessage toWebhook(TextChannel channel) {
        return new chikachi.discord.core.WebhookMessage(
    ...
    private String formatText(String text, Channel channel, boolean isDiscord) {
        ...
        if (channel != null) {
            if (message.contains("@")) {
                Matcher m = chikachi.discord.core.Patterns.tagPattern.matcher(message);
                ...
                while (m.find()) {
                    ...
                    if (Configuration.getConfig().minecraft.dimensions.generic.canMentionUsers) {
                        if (channel instanceof GuildChannel) {
                            ...
                            Optional<Member> theMember = guildChannel.getGuild().getMembersByName(name, true)
                                .stream()
                                .filter(member -> hash == null || member.getUser().getDiscriminator().equalsIgnoreCase(hash))
                                .filter(member -> member.hasPermission(guildChannel, Permission.VIEW_CHANNEL))
                                .findAny();
                            ...
                            DiscordIntegrationLogger.Log("Can't handle channel, since it's not a guild, but is: " + channel.getClass() + " (" + channel.getName() + "," + channel.getType() + ")");
                        }
                    }
                        ...
                        if (channel instanceof GuildChannel) {
                            ...
                            Optional<Role> theRole =
                                guildChannel
                                    ...
                                    .filter(role -> role.hasPermission(guildChannel, Permission.VIEW_CHANNEL))
                                    .findAny();
        ...
        if (this.isParsing()) {
            if (isDiscord) {
                message = chikachi.discord.core.CoreUtils.replace(CoreConstants.minecraftToDiscordEmotes, message);
                message = EmojiParser.parseToUnicode(message);
                message = chikachi.discord.core.Patterns.minecraftToDiscord(message);
            } else {
